...
output_filename = '1_resample_df.csv'
...
df2 = df.resample('Q-NOV', on='DateTime_US_Pacific').mean()
plot_start_time = datetime(2020, 5, 31)
#plot_end_time = datetime(2020, 8, 31)
df2.reset_index(inplace=True)
df2 = df2[df2['DateTime_US_Pacific'] == plot_start_time]
#df2 = df2[(df2['DateTime_US_Pacific'] >= plot_start_time) & (df2['DateTime_US_Pacific'] <= plot_end_time)]
...
df2 = df2[cols]
df2 = df2.dropna()
df2['PM25'] = df2['PM25'].astype(int)
#print(df2)
df2.to_csv(output_full_file_path)
fig_num = plot_map(root_path, df2, map_plt, 1, start_time, bbox_plot, 'NASA', range, marker, map)